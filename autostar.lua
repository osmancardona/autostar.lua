-- Anime Astral Simulator | Auto Open Stars v4
local LP = game:GetService("Players").LocalPlayer
local PG = LP:WaitForChild("PlayerGui")
local UIS = game:GetService("UserInputService")

local autoOn, count = false, 0
local minimized = false

-- Islas detectadas del juego
local ISLANDS = {
...
-- ► Funciones para obtener botones del juego
local function getOpenBtn()
    return PG:FindFirstChild("Windows", true) and
        PG.Windows:FindFirstChild("Star", true) and
        PG.Windows.Star:FindFirstChild("Open", true)
end

local function getAutoBtn()
    return PG:FindFirstChild("Windows", true) and
        PG.Windows:FindFirstChild("Star", true) and
        PG.Windows.Star:FindFirstChild("Auto", true)
end

-- Ruta directa más confiable
local function clickOpen()
    local ok = pcall(function()
        local btn = PG.Windows.Star.Content.Buttons.Open
    ...
    if not ok then
        ...
        pcall(function()
            for _, v in ipairs(PG:GetDescendants()) do
                if v.Name == "Open" and (v:IsA("TextButton") or v:IsA("ImageButton")) then
...
-- ICONO flotante (para mostrar cuando está minimizado)
...
Instance.new("UIStroke", f).Color = Color3.fromRGB(255,200,0)
Instance.peek(f):FindFirstChildOfClass("UIStroke").Thickness = 2
...
-- CONTENIDO (frame interno para minimizar)
...
for i, island in ipairs(ISLANDS) do
    ...
    wb.MouseButton1Click:Connect(function()
        ...
        islandLbl.Text = "🗺️ Isla: " .. island.name:gsub("[%z\1-\127\194-\244][\128-\191]*", function(c)
...
resetBtn.Position = UDim2.new(0,5,0,362)
